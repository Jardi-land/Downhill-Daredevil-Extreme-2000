...
class Camera:
    ...
    def __init__(self, player_image, player_spawn_pos, player_rect, player_track) -> None:
        ...
        self.surface_track = pygame.Surface(
            (1920, self.player_pos.y - self.player_image.get_height()/2)).convert_alpha()
        self.surface_track.fill((0, 0, 0, 0))
        self.surface_track_full = pygame.Surface(
            (1920, self.player_pos.y - self.player_image.get_height()/2)).convert_alpha()
    ...
    def draw(self, chunk_list):
        ...
        self.surface.blit(self.player_track, (self.player_pos.x - self.player_image.get_width() /
                          2, self.player_pos.y - self.player_image.get_height()/2))
        # Get track moving surface_track
        self.surface_track.blit(self.player_track, (self.player_pos.x - self.player_image.get_width(
        )/2, self.player_pos.y - self.player_image.get_height()/2 - self.player_speed))
        # Copy full track and blit higher
        self.surface_track_full_copy = self.surface_track_full.copy()
        self.surface_track_full.fill((0, 0, 0, 0))
        self.surface_track_full.blit(
            self.surface_track_full_copy, (0, -self.player_speed))
        ...
        self.surface.blit(self.player_image, (self.player_pos.x - self.player_image.get_width() /
                          2, self.player_pos.y - self.player_image.get_height()/2))
        ...
        for value, chunk in enumerate(chunk_list):
            if self.vector_offset.y + (value*chunk.surfaces["up"].get_height()) > 2000 or self.vector_offset.y + (value*chunk.surfaces["up"].get_height()) < -2000:
                ...
                for rect in chunk.colliders_list:
                    ...
                    rect["rect"].y = rect["pos"][1] + \
                        (self.vector_offset.y +
                         (value*chunk.surfaces["up"].get_height()))
                ...
                if chunk.bonus:
                    ...
                    chunk.speed_minus_rect.y = chunk.speed_minus_pos[1] + (
                        self.vector_offset.y + (value*chunk.surfaces["up"].get_height()))
                    ...
                    if self.debug.debug_mode:
                        pygame.draw.rect(
                            self.surface, (0, 255, 0), chunk.speed_minus_rect)
