...
class Map:
    ...
    def __draw(self):
        for layer in self.layers:
            match layer:
                case "bonus":
                    self.speed_minus_rect = pygame.rect.Rect(self.map_data.get_object_by_name("speed_minus").x*self.scale, self.map_data.get_object_by_name(
                        "speed_minus").y*self.scale, self.map_data.get_object_by_name("speed_minus").width*self.scale, self.map_data.get_object_by_name("speed_minus").height*self.scale)
                    self.speed_minus_pos = (self.map_data.get_object_by_name(
                        "speed_minus").x*self.scale, self.map_data.get_object_by_name("speed_minus").y*self.scale)
                    self.bonus = True
                case other:
        ...
        for gid, tile_obj_g in self.map_data.get_tile_colliders():
            for collider in tile_obj_g:
                for x, y, layer_index in self.map_data.get_tile_locations_by_gid(gid):
                    ...
                    self.colliders_list.append({"pos": (x*16*self.scale + collider.x*self.scale, y*16*self.scale + collider.y*self.scale), "rect": pygame.Rect(
                        collider.x*self.scale, collider.y*self.scale, collider.width*self.scale, collider.height*self.scale)})
